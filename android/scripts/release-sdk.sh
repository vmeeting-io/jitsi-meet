...
# Now build and publish the Vmeeting SDK and its dependencies
echo "Building and publishing the Vmeeting SDK"
pushd ${THIS_DIR}/../
./gradlew clean 
...
if [[ $DO_GIT_TAG == 1 ]]; then
    ...
    pushd ${MVN_REPO_PATH}
    git add -A .
    git commit -m "Vmeeting SDK + dependencies: ${SDK_VERSION}"
    popd
