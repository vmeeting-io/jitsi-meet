...
export function getModeratorTabProps(stateful: Object | Function) {
    ...
    return {
        ...
        whiteboardUserVisible: Boolean(roomInfo?.whiteboard?.userVisible),
        whiteboardUseYN: Boolean(roomInfo?.whiteboard?.use_yn)
    };
}
...
export function getProfileTabProps(stateful: Object | Function) {
    ...
    let { hideEmailInSettings, useLogin } = config;
 ...
 * dialog.
 */
export function getNotificationsTabProps(stateful: Object | Function, showSoundsSettings: boolean) {
    const state = toState(stateful);
    const {
