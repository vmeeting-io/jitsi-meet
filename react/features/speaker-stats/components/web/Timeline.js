...
const Timeline = ({ faceLandmarks }) => {
    const startTimestamp = useSelector((state) => getConferenceTimestamp(state)) ?? 0;
    const { left, right } = useSelector((state) => getTimelineBoundaries(state));
    const { timelinePanning } = useSelector((state) => state['features/speaker-stats']);
